...
export default function Page() {
  ...
  const startRun = async () => {
    ...
    try {
      ...
      const resProject = await fetch("/api/project/ensure", {
        method: "POST",
        cache: "no-store",
      });
      ...
      if (!resProject.ok || !projectId)
        throw new Error(ensureErr || "Failed to ensure project");
      ...
      const { runId, error: startErr } = (await res.json()) as {
        runId?: string;
        error?: string;
      };
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-gray-600">
        Kick off an analysis to collect buyer conversations and comparisons.
      </p>
      <button
        onClick={startRun}
        disabled={loading}
